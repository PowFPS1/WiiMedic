...
static void test_gc_controllers(void) {
    ...
    for (port = 0; port < 4; port++) {
        ...
        bool connected = (stickX || stickY || cstickX || cstickY ||
                          btns || trigL || trigR);
        ...
        if (connected) {
            ...
            /* Button display */
            printf("   " UI_CYAN "  Buttons " UI_RESET "................. ");
            if (btns & PAD_BUTTON_A)     printf(UI_BGREEN "A " UI_RESET);
            if (btns & PAD_BUTTON_B)     printf(UI_BGREEN "B " UI_RESET);
            if (btns & PAD_BUTTON_X)     printf(UI_BGREEN "X " UI_RESET);
            if (btns & PAD_BUTTON_Y)     printf(UI_BGREEN "Y " UI_RESET);
            if (btns & PAD_TRIGGER_Z)    printf(UI_BGREEN "Z " UI_RESET);
            if (btns & PAD_BUTTON_START) printf(UI_BGREEN "START " UI_RESET);
            if (btns == 0)               printf(UI_WHITE "(none held)" UI_RESET);
            printf("\n");
            ...
            {
                float cstick_dist = sqrtf((float)(cstickX * cstickX + cstickY * cstickY));
                if (cstick_dist > 15.0f && btns == 0) {
                    snprintf(buf, sizeof(buf),
                             "C-Stick drift detected (distance: %.0f)", cstick_dist);
                    ui_draw_warn(buf);
                }
            }
...
static void test_wiimotes(void) {
    ...
    for (chan = 0; chan < 4; chan++) {
        ...
        if (ret == WPAD_ERR_NONE) {
            ...
            const char *ext_name;
            switch (type) {
                case WPAD_EXP_NONE:        ext_name = "No Extension"; break;
                case WPAD_EXP_NUNCHUK:     ext_name = "Nunchuk"; break;
                case WPAD_EXP_CLASSIC:     ext_name = "Classic Controller"; break;
                case WPAD_EXP_GUITARHERO3: ext_name = "Guitar Hero Controller"; break;
                case WPAD_EXP_WIIBOARD:    ext_name = "Balance Board"; break;
                default:                    ext_name = "Unknown Extension"; break;
            }
            ...
            /* Buttons */
            {
                ...
                if (wdata) {
                    ...
                    printf("   " UI_CYAN "  Buttons " UI_RESET "................. ");
                    if (btns & WPAD_BUTTON_A)     printf(UI_BGREEN "A " UI_RESET);
                    if (btns & WPAD_BUTTON_B)     printf(UI_BGREEN "B " UI_RESET);
                    if (btns & WPAD_BUTTON_1)     printf(UI_BGREEN "1 " UI_RESET);
                    if (btns & WPAD_BUTTON_2)     printf(UI_BGREEN "2 " UI_RESET);
                    if (btns & WPAD_BUTTON_PLUS)  printf(UI_BGREEN "+ " UI_RESET);
                    if (btns & WPAD_BUTTON_MINUS) printf(UI_BGREEN "- " UI_RESET);
                    if (btns & WPAD_BUTTON_HOME)  printf(UI_BGREEN "HOME " UI_RESET);
                    if (btns == 0)                printf(UI_WHITE "(none held)" UI_RESET);
                    printf("\n");

                    /* Battery */
                    {
                        float batt_pct = (float)wdata->battery_level / 2.08f;
                        const char *batt_color;
                        if (batt_pct > 100.0f) batt_pct = 100.0f;
                        batt_color = (batt_pct > 50.0f) ? UI_BGREEN :
                                     (batt_pct > 20.0f) ? UI_BYELLOW : UI_BRED;
                        snprintf(buf, sizeof(buf), "%.0f%%", batt_pct);
                    ...
                    if (type == WPAD_EXP_NUNCHUK) {
                        s8 nun_x = wdata->exp.nunchuk.js.pos.x -
                                   wdata->exp.nunchuk.js.center.x;
                        s8 nun_y = wdata->exp.nunchuk.js.pos.y -
                                   wdata->exp.nunchuk.js.center.y;
                        float nun_dist;
                        ...
                        if (nun_dist > 15.0f) {
                            snprintf(buf, sizeof(buf),
                                     "Nunchuk stick drift (distance: %.0f)", nun_dist);
                            ui_draw_warn(buf);
                        }
                    }
                }
