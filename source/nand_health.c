...
void run_nand_health(void) {
  ...
  ret = ISFS_Initialize();
  bool we_initialized = (ret >= 0);
  if (ret < 0 && ret != -105) { /* -105 = ISFS_EALREADY */
  ...
  {
    char buf[64];
    snprintf(buf, sizeof(buf), "%u / %u clusters", s_used_blocks,
             (u32)NAND_TOTAL_CLUSTERS);
  ...
  {
    char buf[64];
    snprintf(buf, sizeof(buf), "%u / %u", s_used_inodes,
             (u32)NAND_TOTAL_INODES);
...

  {
    ...
    if (cluster_pct > 95.0f)
      s_health_score -= 30;
    else if (cluster_pct > 85.0f)
      s_health_score -= 15;
    ...
    if (inode_pct > 95.0f)
      s_health_score -= 30;
    else if (inode_pct > 85.0f)
      s_health_score -= 15;
    else if (inode_pct > 75.0f)
      s_health_score -= 5;
  ...
  /* Health score display */
  {
    ...
    if (s_health_score >= 80) {
      ...
      snprintf(score_msg, sizeof(score_msg), "NAND Health Score: %d/100 - %s",
...
void get_nand_health_report(char *buf, int bufsize) {
  snprintf(buf, bufsize,
           ...
           s_used_inodes, (u32)NAND_TOTAL_INODES, s_free_inodes, s_title_count,
           s_ticket_count, s_health_score, s_health_status);
}
