...
void run_report_generator(void) {
  ...
  /* Setup save target first so we don't malloc a huge string */
  ...
  snprintf(buf, sizeof(buf), "Size: %ld bytes", report_size);
  ui_draw_ok(buf);
