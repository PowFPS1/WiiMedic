 ...
 * the original is often backed up as 1XXXXXXXX.app (1000000XX.app) in the
 * content dir.
 */
static bool detect_priiloader_nand(void) {
  u64 sys_menu_tid = 0x0000000100000002ULL;
  u32 tmd_size = 0;
  ...
  if (ES_GetStoredTMDSize(sys_menu_tid, &tmd_size) < 0 || tmd_size == 0)
    goto cleanup;

  stmd = (signed_blob *)memalign(32, (tmd_size + 31) & ~31);
  if (!stmd)
    goto cleanup;

  if (ES_GetStoredTMD(sys_menu_tid, stmd, tmd_size) < 0)
    goto cleanup;

  tmd *tmd_ptr = (tmd *)SIGNATURE_PAYLOAD(stmd);
  u16 boot_index = tmd_ptr->boot_index;

  if (boot_index < tmd_ptr->num_contents) {
    u32 content_id = tmd_ptr->contents[boot_index].cid;
    char path[ISFS_MAXPATH];
    /* The backup is Content ID + 0x10000000 in the system menu folder */
...
/*
 * BootMii-as-boot2 compatibility is determined by boot1 (boot1a/b = yes,
 * boot1c/d = no). Read OTP boot1 hash via Hollywood registers when PPC has
 * AHBPROT (e.g. launched from HBC). Returns: 1 = compatible (boot1a/b), 0 = not
 * compatible (boot1c/d), 2 = unknown hash, -1 = OTP not readable.
 */
/* PPC Hollywood regs: 0xCD000000 is the uncached physical mapping for Wii regs
 * on PPC */
...
static int get_boot1_bootmii_compatible(void) {
  ...
  /* Check for AHBPROT disabled - usually register 0x0D800064 returns 0xFFFFFFFF
   * when disabled via HBC. */
  if (hw[HW_AHBPROT_OFF / 4] != 0xFFFFFFFF)
    return -1;

  /* Read OTP words 0-4 (boot1 hash, 20 bytes). */
  u8 hash[20];
  unsigned int i;
  for (i = 0; i < 5; i++) {
    /* Trigger read and wait for hardware to complete (bit 31 clears) */
    hw[HW_OTPCMD_OFF / 4] = OTP_RD_BIT | i;
    while (hw[HW_OTPCMD_OFF / 4] & OTP_RD_BIT)
      ;
...
void run_system_info(void) {
  ...
  /* Brick Protection. BootMii (boot2): use boot1 from OTP when AHBPROT
   * available, else fall back to boot2 version proxy. */
  ui_draw_section("Brick Protection");
  {
    ...
    bool has_bootmii_boot2 =
        (boot1_ok == 1) || (boot1_ok < 0 && boot2_suggests_bootmii_ok);
    ...
    if (has_priiloader) {
      ...
      ui_draw_info(
          "If installed in NAND, add apps/priiloader to this SD/USB to detect");
    }
...
void get_system_info_report(char *buf, int bufsize) {
...

  {
    ...
    bool has_bootmii_boot2 =
        (boot1_ok == 1) || (boot1_ok < 0 && boot2_suggests_ok);
    bool has_bootmii_ios = detect_bootmii_ios();
...

    {
      ...
      snprintf(buf, bufsize,
               ...
               get_progressive_string(), hollywood_ver, device_id,
               boot2_version, ios_ver, ios_rev, mem1_size / 1024,
               mem2_size / 1024, has_priiloader ? "Installed" : "Not found",
